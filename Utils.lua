...
function DpsHelper.Utils:Print(message)
    if DpsHelper.Config:Get("enableDebug") then
        print("|cFF33FF99DpsHelper:|r " .. message)
    end
end

function DpsHelper.Utils:MarkSpellUsed(spellName)
    DpsHelper.Utils.RecentSpells[spellName] = GetTime()
end

function DpsHelper.Utils:IsSpellRecentlyUsed(spellName)
    local lastUsed = DpsHelper.Utils.RecentSpells[spellName] or 0
    return (GetTime() - lastUsed) < 2
end

function DpsHelper.Utils:UpdateAuraCache(unit)
    if not UnitExists(unit) then return end
    local cache = DpsHelper.Utils.AuraCache[unit] or {}
    for i = 1, 40 do
    ...
    DpsHelper.Utils.AuraCache[unit] = cache
end
...
function DpsHelper.Utils:GetCombatTime()
    return DpsHelper.Utils.combatStartTime and (GetTime() - DpsHelper.Utils.combatStartTime) or 0
end
...
function DpsHelper.Utils:GetNumberOfEnemiesNearby(range)
    ...
    for i = 1, 40 do
        ...
        if UnitExists(unit) and UnitCanAttack("player", unit) and not UnitIsDeadOrGhost(unit) then
            local distance = DpsHelper.Utils:GetDistanceToUnit(unit)
