...
function DpsHelper.TalentManager:DetectSpec()
    ...
    DpsHelper.Utils:DebugPrint(1,"Detecting specialization for class: " .. playerClass)
    ...
    if tabCount > 0 then
        DpsHelper.Utils:DebugPrint(1,"Scanning talent trees...")
        for tabIndex = 1, tabCount do
            ...
            DpsHelper.Utils:DebugPrint(1,string.format("Tree %s (%d): %d points", name or "Unknown", tabIndex, pointsSpent))
        ...
        if maxPoints > 0 then -- Qualquer árvore com pontos é considerada
            ...
            DpsHelper.Utils:DebugPrint(1,"Detected specialization by points: " .. DpsHelper.Config:Get("currentSpec") .. " (" .. maxPoints .. " points)")
            return detectedSpec
        else
            DpsHelper.Utils:DebugPrint(1,"No points found in any talent tree, defaulting to unknown")
        end
    else
        DpsHelper.Utils:DebugPrint(1,"No talent tabs available, defaulting to unknown")
    end

    -- Default para "unknown" se nenhuma árvore tiver pontos
    DpsHelper.Utils:DebugPrint(1,"No specialization detected, defaulting to unknown")
    DpsHelper.Config:Set("currentClass", playerClass:lower())
    return detectedSpec
end

DpsHelper.Utils:DebugPrint(1,"TalentManager.lua initialized")